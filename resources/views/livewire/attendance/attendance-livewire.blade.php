<div>
    <nav class="navbar navbar-expand-lg navbar-dark bg-mid-bar border-bottom-0 pb-1">
		<div class="input-group col-md-6 mb-1 px-0">
			<div class="input-group rounded">
				<input type="search" class="form-control rounded btn-white border-white" placeholder="Search Attendance" wire:model.debounce.1000ms='search'/>
    ...
    <div class="row mx-1">
		<div class="contents-container col-12">
			@include('livewire.attendance.attendance-search')
		</div>
	</div>

    @can('create', \App\Models\Department::class)
        <div wire:ignore>
            @livewire('attendance.attendance-edit-livewire'))
        </div>
    @endcan
    ...
    <script>
        ...
        window.addEventListener('swal:confirm:delete_attendance', event => { 
            swal({
				...
				if (willDelete) {
					@this.delete_attendance(event.detail.attendance_id)
				}
            });
        });
    </script>
